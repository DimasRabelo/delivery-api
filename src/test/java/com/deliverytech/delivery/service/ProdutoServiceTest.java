...
import java.util.List; // IMPORT ADICIONADO (para testar o setter)
...
class ProdutoTest {
    ...
    void should_CreateProduto_When_DefaultConstructor() {
        ...
        // (Seu código original dizia que 'itensPedido' era inicializado)
        assertNotNull(produto.getItensPedido()); 
        assertTrue(produto.getItensPedido().isEmpty()); 
        
        // (Assumindo que 'gruposOpcionais' também é inicializado na entidade)
    ...
    void should_SetAndGetProperties_When_ValidValues() {
        // Given
        Produto produto = new Produto();
        BigDecimal precoBase = new BigDecimal("25.99"); // <-- Renomeado
        Restaurante restaurante = new Restaurante();
        restaurante.setId(10L);
        List<GrupoOpcional> grupos = new ArrayList<>(); // <-- Novo
